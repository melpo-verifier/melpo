...
for (let i = 0; i < args.length; i += 1) {
  if (args[i] === "--clientId" && args[i + 1]) {
    clientIdArg = String(args[i + 1]).trim().replace(/^"|"$/g, "");
    i += 1;
    continue;
  }
  ...
  if (!args[i].startsWith("--") && !botNameArg) {
    botNameArg = args[i];
  }
}
...
async function resolveDeployCredentials() {
  if (clientIdArg) {
    ...
    if (!instance || !instance.bot_token) {
      throw new Error(`No instance token found for client ID ${clientIdArg}.`);
    }
  ...
  if (!token || !applicationId) {
    throw new Error(`Missing ${botName}_TOKEN or ${botName}_ID in environment.`);
  }
...
(async () => {
  try {
    ...
    if (shouldClear) {
      console.log(`Started clearing global application (/) commands for ${applicationId}.`);
    } else {
      console.log(
    ...
    if (shouldClear) {
      console.log(`Successfully cleared global application (/) commands for ${applicationId}.`);
    } else {
      console.log(
