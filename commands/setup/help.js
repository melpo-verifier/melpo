...
module.exports = {
	data: new SlashCommandBuilder().setName("help").setDescription("Get a list of commands").setContexts(0),
	async execute({ interaction, client }) {
		...
		for (const folder of orderedFolders) {
			...
			if (subcommands.length > 0) {
				catagoryEmbed.addFields({
					name: "Subcommands",
					value: subcommands?.map((command) => `/${command.data.name}`).join("\n"),
				});
			}
		...
		await buttonPagination(interaction, helpEmbeds, 60000, ["⏪", "⏩"], 0, 0, "help");
	},
};
