	...
	async execute({ interaction, client }) {
		if (
			...
			return interaction.reply({
				content: "In order to set a custom status you have to have a custom version of Melpo *add more info here*",
		...
		if (!name && !type && !status) {
			return interaction.reply({
				content: "You need to provide at least one of the following: name, type, status",
				flags: MessageFlags.Ephemeral,
			});
		}
		...
		if (type) {
			...
			presence.activities[0].type = parseInt(type, 10);
		}
