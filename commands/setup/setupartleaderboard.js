	...
	async execute({ interaction }) {
		if (!interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
			return await interaction.reply({
				content: "You do not have the required permissions (Manage Server) to use this command.",
				flags: MessageFlags.Ephemeral,
			});
		}

		const guildwhitelist = await Whitelist.findOne({ where: { server_id: interaction.guild.id } });
