...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.setDescription("Check if Melpo has the required permissions to function properly")
		.setContexts(0),
	async execute({ interaction }) {
		...
		for (const perm of requiredPermissions) {
			try {
				const hasPermission = botMember.permissions.has(PermissionsBitField.Flags[perm.name?.replace(/ /g, "")]);
		...
		if (botMember.permissions.has(PermissionsBitField.Flags.ViewChannel)) {
			...
			try {
				...
				const dangerousExcessPerms = allPermissionFlags.filter(([name, bit]) => {
					if (!bit) return false;

					const isRequired = requiredPermissions.some((p) => p.name?.replace(/ /g, "") === name);
					return !isRequired && unneededPermissions.includes(name) && botMember.permissions.has(bit);
				});
		...
		for (const app of applications) {
			...
			if (app?.verifychannel) {
				const verifyChannel = interaction.guild.channels.cache.get(app.verifychannel);
			...
			if (app?.reviewchannel) {
				const reviewChannel = interaction.guild.channels.cache.get(app.reviewchannel);
			...
			if (app?.verifylogs) {
				const logsChannel = interaction.guild.channels.cache.get(app.verifylogs);
			...
			if (app?.verificationwelcomechannel) {
				const welcomeChannel = interaction.guild.channels.cache.get(app.verificationwelcomechannel);
		...
		for (const app of applications) {
			...
			if (app?.unverifiedrole && app.unverifiedrole.length > 0) {
				const unverifiedRoles = Array.isArray(app.unverifiedrole) ? app.unverifiedrole : [app.unverifiedrole];
			...
			if (app?.autorole && app.autorole.length > 0) {
				const autoRoles = Array.isArray(app.autorole) ? app.autorole : [app.autorole];
			...
			if (app?.managerrole && app.managerrole.length > 0) {
				const managerRoles = Array.isArray(app.managerrole) ? app.managerrole : [app.managerrole];
		...
		const missingPermissions = requiredPermissions.filter((perm) => {
			try {
				return !botMember.permissions.has(PermissionsBitField.Flags[perm.name?.replace(/ /g, "")]);
			} catch {
				return true;
			}
		});
