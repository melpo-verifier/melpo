	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand
				...
				.addStringOption((option) =>
					option.setName("title").setDescription("Embed title").setMaxLength(256).setRequired(true),
				)
				.addStringOption((option) =>
					option.setName("description").setDescription("Embed description").setMaxLength(4096).setRequired(true),
				)
				.addStringOption((option) =>
					option
						.setName("color")
				...
				.addAttachmentOption((option) => option.setName("image").setDescription("Embed image").setRequired(false))
				.addAttachmentOption((option) =>
					option.setName("thumbnail").setDescription("Embed thumbnail image").setRequired(false),
				)
				.addStringOption((option) =>
					option.setName("footer").setDescription("Embed footer text").setMaxLength(2048).setRequired(false),
				)
				.addBooleanOption((option) =>
					option.setName("timestamp").setDescription("Add current timestamp?").setRequired(false),
				)
		...
		.addSubcommand((subcommand) =>
			subcommand
				...
				.addStringOption((option) =>
					option.setName("message-id").setDescription("ID of message to edit").setRequired(true),
				)
				.addChannelOption((option) =>
					option
						.setName("channel")
				...
				.addStringOption((option) =>
					option.setName("description").setDescription("Embed description").setMaxLength(4096).setRequired(false),
				)
				.addStringOption((option) =>
					option
						.setName("color")
		...
		.addSubcommand((subcommand) =>
			subcommand
				...
				.addBooleanOption((option) =>
					option.setName("remove-title").setDescription("Remove the title?").setRequired(false),
				)
				.addBooleanOption((option) =>
					option.setName("remove-description").setDescription("Remove the description?").setRequired(false),
				)
				.addBooleanOption((option) =>
					option.setName("remove-color").setDescription("Reset color to default?").setRequired(false),
				)
				.addBooleanOption((option) =>
					option.setName("remove-image").setDescription("Remove the image?").setRequired(false),
				)
				.addBooleanOption((option) =>
					option.setName("remove-thumbnail").setDescription("Remove the thumbnail?").setRequired(false),
				)
				.addBooleanOption((option) =>
					option.setName("remove-footer").setDescription("Remove the footer?").setRequired(false),
				),
		)
	...
	async execute({ interaction }) {
		...
		if (interaction.options.getSubcommand() === "create") {
			...
			if (interaction.options.getString("color"))
				embed.setColor(String("#").concat(interaction.options.getString("color")));
			if (interaction.options.getAttachment("thumbnail"))
				embed.setThumbnail(interaction.options.getAttachment("thumbnail").url);
			if (interaction.options.getAttachment("image")) embed.setImage(interaction.options.getAttachment("image").url);
			...
			if (!channel.permissionsFor(interaction.client.user).has(["ViewChannel", "SendMessages", "EmbedLinks"]))
				return interaction.editReply({ content: `I don't have permission to send embeds in ${channel}!` });

			await channel.send({ embeds: [embed] });

			await interaction.editReply({ content: `Your embed has been sent to ${channel}:`, embeds: [embed] });
		} else if (interaction.options.getSubcommand() === "edit") {
			const channel = interaction.options.getChannel("channel") || interaction.channel;
			const messageId = interaction.options.getString("message-id");

			const message = await channel.messages.fetch(messageId).catch(() => null);
			if (!message) return interaction.editReply("Could not find that message!");

			if (message.author.id !== interaction.client.user.id)
				return interaction.editReply("I can only edit my own embeds!");
			if (!message.embeds?.[0]) return interaction.editReply("That message doesn't contain an embed!");
			...
			const updates = {
				...
				color: String("#").concat(interaction.options.getString("color")),
			...
			if (updates.timestamp !== null) updates.timestamp ? newEmbed.setTimestamp() : newEmbed.setTimestamp(null);
