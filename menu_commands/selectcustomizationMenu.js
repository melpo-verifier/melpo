...
module.exports = async ({ interaction, customIdValue, tempApplicationId, context }) => {
	let chosenvalue;
	if (customIdValue) chosenvalue = customIdValue;
	else chosenvalue = interaction.values[0];
	...
	const title =
		tempApp[chosenvalue]?.title === "deleted"
			? null
			: (tempApp[chosenvalue]?.title || applicationSetup?.[chosenvalue]?.title)?.replace(
					// biome-ignore lint/suspicious/noTemplateCurlyInString: Template like string subsitution.
	...
	const description =
		tempApp[chosenvalue]?.description === "deleted"
			? null
			: (tempApp[chosenvalue]?.description || applicationSetup?.[chosenvalue]?.description)?.replace(
					// biome-ignore lint/suspicious/noTemplateCurlyInString: Template like string subsitution.
					"${interaction.guild.name}",
					interaction.guild.name,
				) || null;
	...
	const image =
		tempApp[chosenvalue]?.image === "deleted"
			? null
			: tempApp[chosenvalue]?.image || applicationSetup?.[chosenvalue]?.image || null;

	const text =
		tempApp[chosenvalue]?.text === "deleted"
			? null
			: tempApp[chosenvalue]?.text || applicationSetup?.[chosenvalue]?.text || null;

	const setImage = new ActionRowBuilder().addComponents(
		new ButtonBuilder({
			custom_id: `setText_${chosenvalue}_${tempApplicationId}`,
			label: "Set Text",
			style: ButtonStyle.Primary,
		}),
		new ButtonBuilder({
			custom_id: `setImage_${chosenvalue}_${tempApplicationId}`,
			label: "Set Image",
			style: ButtonStyle.Primary,
		}),
		new ButtonBuilder({
			custom_id: `removeImage_${chosenvalue}_${tempApplicationId}`,
			label: "Remove Image",
			style: ButtonStyle.Danger,
		}),
		new ButtonBuilder({
			custom_id: `resetText_${chosenvalue}_${tempApplicationId}`,
			label: "Reset Text",
			style: ButtonStyle.Danger,
		}),
	);
	...
	if (infoembed) infoembed.setColor("#3f7ff1");

	if (image && embed) {
		const asset = resolveImage(image);
		if (asset.embedUrl) embed.setColor(isValidHexColor(color) ? color : "#3f7ff1").setImage(asset.embedUrl);
	}

	if (embed !== undefined) embed.setFooter({ text: `This is the ${chosenvalue}.` });
	...
	if (interaction.replied || interaction.deferred) {
		if (chosenvalue === "verificationwelcomemessage" || chosenvalue === "verifychannelembed") {
			interaction.message.edit({
				content: text ? text : null,
				embeds: embed ? [infoembed, embed] : [infoembed],
				components: [interaction.message.components[0], selectcustomizationMenu, colourmenu, setImage, finishbuttons],
			});
		} else {
			interaction.message.edit({
				content: text ? text : null,
				embeds: infoembed ? [infoembed, embed] : [embed],
				components: [interaction.message.components[0], selectcustomizationMenu, setImage, finishbuttons],
			});
		}
	} else {
		if (chosenvalue === "verificationwelcomemessage" || chosenvalue === "verifychannelembed") {
