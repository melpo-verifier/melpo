...
module.exports = async ({ interaction, context }) => {
	...
	if (num === 1) {
		...
		collector.on("collect", async (m) => {
			...
			if (emotes(m.content) === null) return m.reply("Please enter a valid emoji to use as the reaction emoji.");
			...
			const skipandfinish = new ActionRowBuilder().addComponents(
				new ButtonBuilder().setCustomId("artchannelfinish").setLabel("Skip & Finish").setStyle("Primary"),
			);
	...
	if (num === 3) await interaction.editReply({ content: `Great! The setup is now completed!`, components: [] });
};
