...
async function updateDatabaseEntry(applicationId, category, s3ImageData) {
	try {
		...
		if (!promotedImage) throw new Error(`Failed to promote image to final scope for category ${category}`);
		...
		if (!serializedImage?.key) throw new Error(`Failed to serialize promoted image for category ${category}`);
