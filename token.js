...
if (!action || !name || (action === "add" && (!token || !clientId))) {
	console.error("Usage for add:    node token.js add <name> <token> <clientId>");
...
if (action === "add") {
	...
	for (const line of lines) {
		if (line.startsWith(`${upperName}_TOKEN=`) || line.startsWith(`${upperName}_ID=`)) {
			found = true;
		}
	}

	if (found) {
		console.log(`Bot name already exists with id: ${process.env[`${upperName}_ID`]}.`);
	} else {
