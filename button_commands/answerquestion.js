...
module.exports = async ({ interaction, client }) => {
	...
	const info = await QuestionId.findOne({ where: { interactionMessageId: interaction.message.id } });
	...
	const confirmrow = new ActionRowBuilder().addComponents(
		...
		// new ButtonBuilder().setCustomId("opt-out").setLabel("Opt-out").setStyle("Danger"),
	);

	const disabledconfirmrow = new ActionRowBuilder().addComponents(
		new ButtonBuilder().setCustomId("answerquestion").setLabel("Answer").setStyle("Success").setDisabled(true),
		// new ButtonBuilder().setCustomId("opt-out").setLabel("Opt-out").setStyle("Danger").setDisabled(true),
	);

	const replybutton = new ActionRowBuilder().addComponents(
		new ButtonBuilder().setCustomId(`question_${applicationId}_${user.id}`).setLabel("Reply").setStyle("Primary"),
	);
	...
	await QuestionId.destroy({ where: { interactionMessageId: interaction.message.id } });
	...
	messageCollector.on("collect", async (m) => {
		...
		if (totalcontent.length + questionform.fields[0].value.length > 3800)
			totalcontent = String(totalcontent.substring(0, 3800 - questionform.fields[0].value.length)).concat("...");
		...
		if (m.attachments.size > 0) {
			...
			if (mediaItems.length > 0) {
				...
				container.addMediaGalleryComponents(new MediaGalleryBuilder({ items: mediaItems }));
			}
		}
		...
		await sendanswer.edit({ content: "", components: [], embeds: [answerform] });
				...
				async (c, { verificationChannelId, verificationMessageId, replybutton, container, channelId }) => {
					...
					if (verificationChannelId !== channelId) {
						...
						try {
							...
							console.error("Error fetching verification message or thread:", error);
						}
					}
