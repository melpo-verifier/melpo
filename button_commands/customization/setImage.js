...
const { uploadCustomizationImage, purgeOldImages, serializeImage } = require("../../js/customizationImages.js");
const { getTempApplicationById, updateTempApplication } = require("../../js/tempconfigfuncs.js");
...
module.exports = async ({ interaction, context }) => {
	...
	if (!customIdValue || Number.isNaN(tempApplicationId)) {
	...
	const imageaskembed = new EmbedBuilder()
		.setTitle("Set Image")
		.setDescription("Please upload an image or paste an image URL within 30 seconds to update the image.")
		.setColor("#3f7ff1");
	...
	collector.on("collect", async (collected) => {
		try {
			...
			await updateTempApplication(
				...
				{ [customIdValue]: { image: storedImage } },
...
async function refreshCustomizationEmbed({ interaction, image }) {
	...
	await interaction.message.edit({ embeds: embedsToSend, files: [] });
}
...
async function fetchImage(url) {
	...
	if (!response.ok) throw new Error("Failed to fetch image. Make sure the URL is public and valid.");
	...
	return { buffer, contentType, extension: ALLOWED_TYPES[contentType] };
}
