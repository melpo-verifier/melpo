...
module.exports = async ({ interaction, context, whichdefault, applicationId, tempApplicationId, client }) => {
	...
	const { tempApp, error } = await getTempApplicationById(parseInt(tempApplicationId, 10), interaction.guild.id);
	if (error) {
		return interaction.reply({
			content: `Error: ${error}`,
			flags: MessageFlags.Ephemeral,
	...
	if (context && context[1] === "true") {
		await deleteTempApplication(interaction.guild.id, { id: parseInt(tempApplicationId, 10) });
	...
	const { tempApp: finalTempApp } =
		tempApplicationId !== tempApp?.id
			? await getTempApplicationById(parseInt(tempApplicationId, 10), interaction.guild.id)
			: { tempApp };
	...
	const reviewChannel =
		finalTempApp.reviewchannel === "deleted" ? null : finalTempApp.reviewchannel || applicationSetup?.reviewchannel;
	const verifyLogsChannel =
		finalTempApp.verifylogs === "deleted" ? null : finalTempApp.verifylogs || applicationSetup?.verifylogs;
	const verifyChannel =
		finalTempApp.verifychannel === "deleted" ? null : finalTempApp.verifychannel || applicationSetup?.verifychannel;
	const verificationwelcomechannel =
		finalTempApp.verificationwelcomechannel === "deleted"
			? null
			: finalTempApp.verificationwelcomechannel || applicationSetup?.verificationwelcomechannel;
	...
	if (Math.random() < 0.05) {
		...
		if (!isPaidUser) {
			const adTexts = await AdTexts.findAll({ type: "setup" });
		...
		.addFields(
			{
				...
				value: verifyChannel ? `<#${verifyChannel.toString()}>` : `**Not set up**`,
				inline: false,
			},
			{
				name: "Verification Review Channel `required`",
				value: reviewChannel ? `<#${reviewChannel.toString()}>` : `**Not set up**`,
				inline: false,
			},
			{
				name: "Verification Logs Channel `optional`",
				value: verifyLogsChannel ? `<#${verifyLogsChannel.toString()}>` : `**Not set up**`,
				inline: false,
			},
			{
