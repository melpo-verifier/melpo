...
module.exports = async ({ interaction, client, userid, applicationId }) => {
	...
	const replybutton = new ActionRowBuilder().addComponents(
		new ButtonBuilder().setCustomId(`question_${applicationId}_${userid}`).setLabel("Reply").setStyle("Primary"),
	);

	const confirmrow = new ActionRowBuilder().addComponents(
		new ButtonBuilder().setCustomId(`answerquestion_${applicationId}`).setLabel("Answer").setStyle("Success"),
	...
	try {
		...
		const questionsend = await user.send({ embeds: [questionform], components: [confirmrow] });
