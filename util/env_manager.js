...
class env_manager {
  ...
  static config() {
    ...
    if (env_manager.#cfg.flags & 0x04) {
      if (
        ...
        (env_manager.#cfg.flags != parseInt(process.env[env_manager.#cfg.env_name]))
      )                                                                                   // Environment var existing and different from our status?
      { env_manager.#cfg.flags = parseInt(process.env[env_manager.#cfg.env_name]); }      // Adopt it as our new status and continue.
    }

    if (!(env_manager.#cfg.flags & 0x01))          //If our first load has been done, we do not need to reparse.
    {
      //require("dotenv").config();
      try { 
        process.loadEnvFile();                 //Try loading the .env file using node.js's native callback.
        console.log("Loaded .env file.");      //Notify on success.
        env_manager.#cfg.flags |= 0x01;        //Set a flag that we loaded properly, as we do not need to repeat read.
      } 
      catch (error) 
      { 
        if (env_manager.#cfg.flags & 0x02)
        {
          ...
          env_manager.#cfg.flags ^= 0x02;     //xor off the bit as we have already errored.
        }
      }
    }
    ...
    if (env_manager.#cfg.flags & 0x04) { process.env[env_manager.#cfg.env_name]=String(env_manager.#cfg.flags) }
  }
   ...
   */
  static has(elem) { return (typeof(process.env[elem]) !== "undefined"); }
}
