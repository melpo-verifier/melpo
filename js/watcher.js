...
async function clearAndRecacheModule(modulePath, client) {
	...
	if (require.cache[resolvedPath]) {
		...
		try {
			...
			/* Condition checks */
			const isFunction = typeof newModule === "function";
			/*
			const isCommand = (
				typeof newModule !== "undefined" &&
				typeof newModule.data !== "undefined" &&
				typeof newModule.execute !== "undefined");
			*/
			const isCommand =
				typeof newModule !== "undefined" &&
				typeof newModule.data !== "undefined" &&
			...
			} else if (isFunction && modulePath.includes("button_commands")) {
	...
	fs.watch(fullPath, { recursive: true }, (eventType, filename) => {
		if (filename && eventType === "change" && filename.endsWith(".js")) {
			...
			timers[filePath] = setTimeout(() => {
				...
				clearAndRecacheModule(filePath, client).catch(() => {});
