...
async function getApplicationById(applicationId, guildId) {
	if (!applicationId || Number.isNaN(applicationId)) return { application: null, error: "Invalid application ID" };
...
async function getTempApplicationById(tempApplicationId, guildId) {
	...
	if (!tempApp) return { tempApp: null, error: "Temp application not found" };
	if (tempApp.server_id !== guildId) return { tempApp: null, error: "Temp application does not belong to this server" };
	...
	if (appData.applicationId) {
		...
		if (existingApp) {
			...
			defaults = {
				...
				...copyableFields,
				...Object.fromEntries(Object.entries(appData).filter(([key]) => key !== "applicationId")),
			};
		}
	}
