...
const isR2ImageResource = (image) => Boolean(image && typeof image === "object" && image.storage === "r2" && image.key);
...
const buildBasePrefix = (serverId, appName) => `${PREFIX_ROOT}/${serverId}/${sanitizeAppName(appName)}`;
const buildSectionPrefix = ({ serverId, appName, section, scope = "final" }) => {
	const base = buildBasePrefix(serverId, appName);
	if (scope === "temp") return `${base}/${TEMP_FOLDER}/${section}/`;
	return `${base}/${section}/`;
};

const buildKey = ({ serverId, appName, section, extension, scope = "temp", fileId }) => {
...
const isTempKey = (key) => Boolean(key && (key.split("/").includes(TEMP_FOLDER) || key.includes("_temp")));
...
async function uploadCustomizationImage({ serverId, appName, section, buffer, contentType, extension }) {
	if (!Buffer.isBuffer(buffer)) throw new Error("uploadCustomizationImage expects a Buffer");

	const fileId = generateFileId();
	const key = buildKey({ serverId, appName, section, extension, scope: "temp", fileId });
...
async function promoteCustomizationImage(image) {
	...
	return serializeImage({ ...image, key: finalKey, isTemp: false });
}
...
async function listObjectsForPrefix(prefix) {
	...
	do {
		...
		if (response.Contents) objects.push(...response.Contents.filter((obj) => obj?.Key));
...
async function purgeOldImages({ serverId, appName, section, keepKey, filter = "all" }) {
	const objects = await listSectionObjects({ serverId, appName, section });
	const keysToDelete = [];
	const scopeFilter =
		filter === "temp" ? new Set(["temp"]) : filter === "final" ? new Set(["final"]) : new Set(["temp", "final"]);

	for (const object of objects) {
		if (!object.Key) continue;
		if (keepKey && object.Key === keepKey) continue;
		if (!scopeFilter.has(object.scope || (isTempKey(object.Key) ? "temp" : "final"))) continue;
	...
	const chunks = [];
	for (let i = 0; i < keysToDelete.length; i += 1000) chunks.push(keysToDelete.slice(i, i + 1000));
