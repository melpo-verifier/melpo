...
async function sendViaWebhook(webhook, embed, row, name, avatarURL, verifymessage_id, webhookUpdated) {
	...
	const finalMessageId = verifymessage_id;

	try {
		if (webhookUpdated === "true" || !finalMessageId) {
			if (finalMessageId) await client.deleteMessage(finalMessageId).catch(() => {});
...
//Note : Renamed embedConfig to _embedConfig for biome compliance.
async function findVerifyMessage(verifyChannelObj, botId, _embedConfig, applicationId) {
...
async function updateVerifyMessage(opts) {
	const { verifyChannelObj, botId, embedConfig, messageId, appId, webhookUpdated } = opts;
	...
	if (application.mainMessageApplicationId) {
		const mainApp = await Application.findOne({ where: { id: application.mainMessageApplicationId } });
	...
	if (application.branding_enabled === true) {
		...
		if (webhook) {
			try {
				...
				const resultMessageId = await sendViaWebhook(
					webhook,
					embed,
					row,
					name,
...
module.exports = { updateVerifyMessage, isValidHexColor };
