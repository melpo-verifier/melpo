...
//if (typeof(process.env.ENCRYPTION_KEY) === "undefined") throw new Error("No encyption key environment var found.");
if (typeof process.env.ENCRYPTION_KEY === "undefined") throw new Error("No encyption key environment var found.");
...
function decryptData(payloadString) {
	try {
		...
		if (decrypted.toString("utf8").startsWith("{")) return JSON.parse(decrypted.toString("utf8"));
		else return decrypted.toString("utf8");
...
async function getSubmission(messageId) {
	const submission = await Submissions.findOne({ where: { message_id: messageId } });
...
module.exports = { encryptData, decryptData, getLatestSubmissionByUser, getSubmission, isPremiumServer };
