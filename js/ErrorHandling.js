...
class ErrorHandler {
  ...
  static async handle(client, error, interaction = null, context = null) {
    ...
    if (interaction && !interaction.replied && !interaction.deferred) {
      try 
      { await this.handleUserNotification( interaction, errorType, errorId, error ); } 
      catch (userError) 
  ...
  static classifyError(error) {
    ...
    if (
      ...
      (error instanceof SequelizeBaseError) || //Improved sequalise error detection, check if instance of their error base class.
  ...
  static async handleUserNotification(interaction, errorType, errorId, error) {
    ...
    const messages = {
      [this.ERROR_TYPES.PERMISSION]:  "I don't have the required permissions to do that.",
    ...
    if (typeof(process.env.BUG_REPORT_MODE) !== "undefined")
    {
      ...
      const debug_actions = {
        ...
        act_CH: async function(_out) 
        {
          ...
          if (devChan.isThread()) { if (devChan.archived) { await devChan.setArchived(false); } }
      ...
      {
        case "none": break;                                                       //None        : Just skip over sending it discord side.
        case "user": { await debug_actions.act_DM({embeds: [devEmbed]}); } break; //User mode   : DM the issue to the configured userID.
        case "chan": { await debug_actions.act_CH({embeds: [devEmbed]}); } break; //Channel mode: Post the issue to a channel(ideally in a development server).
        case "both":                                                              //Both        : DM the issue and post to a channel.
        {
          await debug_actions.act_DM({embeds: [devEmbed]});
          await debug_actions.act_CH({embeds: [devEmbed]});
        }
        break;
        default: console.log("Unknown report mode : %s", process.env.BUG_REPORT_MODE); break;
      }
    }
  }
  ...
  static cleanupErrorStats() {
    ...
    { if (stats.lastSeen < oneHourAgo) { this.errorStats.delete(type); } }
