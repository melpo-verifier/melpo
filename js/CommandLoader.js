...
class CommandLoader {
	...
	loadAll() {
		...
		(async () => {
			try {
				await Promise.all([this.loadCommands(), this.loadButtonCommands(), this.loadMenus(), this.loadModals()]);
		...
		try {
			...
			for (const folder of commandFolders) {
				try {
					...
					for (const file of commandFiles) this.loadSingleCommand(commandsPath, file, folder);
				} catch (error) {
	...
	loadSingleCommand(commandsPath, file, folder) {
		try {
			...
			if (this.loadedModules.has(filePath)) delete require.cache[require.resolve(filePath)];
			...
			if ("data" in command && "execute" in command) {
				if (!command.data.name) {
					console.warn(`[WARNING] Command at ${filePath} missing name property`);
					return;
				}
	...
	loadButtonCommands() {
		...
		subDirs.forEach(
			(dir) => void this.loadFromDirectory(`./button_commands/${dir}`, this.client.buttonCommands, "button command"),
		);
	}
		...
		try {
			...
			for (const file of files) {
				try {
					...
					if (this.loadedModules.has(absolutePath)) delete require.cache[require.resolve(absolutePath)];
