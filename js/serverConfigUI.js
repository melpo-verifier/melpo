...
const ServerConfigComponent = ({ serverConfig }) => {
	...
	const container = new ContainerBuilder()
		...
		.addTextDisplayComponents((textDisplay) => textDisplay.setContent("## Server Configuration"))
		.addSeparatorComponents((separator) => separator.setSpacing(SeparatorSpacingSize.Small))
		.addTextDisplayComponents(
			(textDisplay) =>
				textDisplay.setContent(
					autoRoles.length ? `**Autoroles**\n${formatRoles(autoRoles)}` : "**Autoroles**\nNot configured",
				),
			(textDisplay) =>
				textDisplay.setContent(
	...
	const exitbutton = new ActionRowBuilder().addComponents(
		new ButtonBuilder().setCustomId("serverconfig_exit").setLabel("Save & Exit").setStyle(ButtonStyle.Primary),
	);
