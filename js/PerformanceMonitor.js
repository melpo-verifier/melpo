...
class PerformanceMonitor extends EventEmitter {
	...
	getCommandStats() {
		...
		for (const [name, stats] of this.metrics.commands.entries()) {
			...
			if (!slowestCommand || stats.averageTime > slowestCommand.averageTime) slowestCommand = { name, ...stats };
		}

		return {
			totalExecutions,
			averageTime: totalExecutions > 0 ? Math.round(totalTime / totalExecutions) : 0,
			totalErrors,
			slowestCommand,
		};
	}
	...
	getInteractionStats() {
		...
		return {
			totalInteractions,
			averageTime: totalInteractions > 0 ? Math.round(totalTime / totalInteractions) : 0,
		};
	}
	...
	cleanup() {
		...
		for (const minute of this.metrics.errors.keys()) if (minute < oneHourAgo) this.metrics.errors.delete(minute);
