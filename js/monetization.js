...
async function handleEntitlementCreate(entitlement, client) {
	console.log(
		`[create] entitlement — skuId=${entitlement?.skuId} guildId=${entitlement?.guildId} userId=${entitlement?.userId}`,
	);
	try {
		if (!entitlement?.skuId) {
			console.log(`[create] No skuId, skipping`);
			return;
		}

		if (!SUBSCRIPTION_SKUS[entitlement.skuId]) {
			console.log(`[create] skuId not found, skipping`);
			return;
		}

		const userId = getUserId(entitlement, client);
		if (!userId) {
			console.log(`[create] Could not resolve userId, skipping`);
			return;
		}
...
async function handleEntitlementUpdate(entitlement, client) {
	console.log(
		`[update] entitlement — skuId=${entitlement?.skuId} guildId=${entitlement?.guildId} userId=${entitlement?.userId}`,
	);
	try {
		if (!entitlement?.skuId) {
			console.log(`[update] No skuId, skipping`);
			return;
		}

		if (!SUBSCRIPTION_SKUS[entitlement.skuId]) {
			console.log(`[update] skuId not found, skipping`);
			return;
		}

		const userId = getUserId(entitlement, client);
		if (!userId) {
			console.log(`[update] Could not resolve userId, skipping`);
			return;
		}
