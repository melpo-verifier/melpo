...
module.exports = async (interaction, pages, time = 30 * 1000) => {
	try {
		if (!interaction || !pages || !pages > 0) throw new Error("Invalid parameters");

		await interaction.deferReply();
		if (pages.length === 1)
			return await interaction.editReply({ embeds: [pages[0]], componennts: [], withResponse: true });

		const prev = new ButtonBuilder().setCustomId("pageprev").setEmoji("⬅️").setStyle("Primary").setDisabled(true);
		//const home = new ButtonBuilder().setCustomId("home").setEmoji('🏠').setStyle("Secondary").setDisabled(true);
		const next = new ButtonBuilder().setCustomId("pagenext").setEmoji("➡️").setStyle("Primary");
