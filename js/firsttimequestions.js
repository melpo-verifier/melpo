...
async function firsttimequestions({ interaction, applicationId, tempApplicationId }) {
	...
	const { tempApp: temporarySetup, error } = await getTempApplicationById(
		parseInt(tempApplicationId, 10),
	...
	const questionbuttons = new ActionRowBuilder().addComponents(
		new ButtonBuilder().setCustomId(`addquestion_1_${tempApplicationId}`).setLabel("Add Question").setStyle("Primary"),
	);
	...
	if (questions && questions.length > 0) {
		...
		else interaction.update({ embeds: [questionembed], components: [editmenu, questionbuttons, finishbuttons] });
	} else {
		if (interaction.replied || interaction.deferred)
			interaction.message.edit({ embeds: [questionembed], components: [questionbuttons, finishbuttons] });
		else interaction.update({ embeds: [questionembed], components: [questionbuttons, finishbuttons] });
	}
}
